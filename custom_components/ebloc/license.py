...
class LicenseManager:
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.warning(
                "[E-Bloc:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[E-Bloc:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[E-Bloc:License] cache expirat — funcționare în perioadă "
                ...
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[E-Bloc:License] cache expirat + perioadă de grație depășită "
